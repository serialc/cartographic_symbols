...
JSVG.createCirc = function(target, id, x, y, r, css_class, actions) {
	...
	circ_elem = document.createElementNS(JSVG.constants.svgNS, 'circle');
...
//creates an ellipse in a target svg object, at x,y with two radii, rx, ry and class css_class, and with actions in object actions
JSVG.createEllipse = function(target, id, x, y, rx, ry, css_class, actions) {
	var oval_elem = document.getElementById(id);
	
	//check if element id already exists and is a ellipse node
	//if so simply modify/update
	if(oval_elem && (oval_elem.tagName || oval_elem.nodeName) === 'ellipse') {
		//if the circle exists simply modify the parameters
		oval_elem.setAttributeNS(null, 'cx', x);
		oval_elem.setAttributeNS(null, 'cy', y);
		oval_elem.setAttributeNS(null, 'rx', rx);
		oval_elem.setAttributeNS(null, 'ry', ry);
		return;
	}

	//create SVG rect and add to target
	oval_elem = document.createElementNS(JSVG.constants.svgNS, 'ellipse');
	oval_elem.setAttributeNS(null, 'id', id);
	oval_elem.setAttributeNS(null, 'cx', x);
	oval_elem.setAttributeNS(null, 'cy', y);
	oval_elem.setAttributeNS(null, 'rx', rx);
	oval_elem.setAttributeNS(null, 'ry', ry);
	oval_elem.setAttributeNS(null, 'class', css_class);
	for (name in actions ) {
		if(oval_elem.addEventListener) {
			oval_elem.addEventListener(name, actions[name], false);
		}
	}

	//Add circle to svg canvas
	document.getElementById(target).appendChild(oval_elem);
};
